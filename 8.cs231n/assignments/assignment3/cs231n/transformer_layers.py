...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # 填充正弦和余弦值
        pe[0, :, 0::2] = torch.sin(position * div_term)  # 偶数列为sin
        pe[0, :, 1::2] = torch.cos(position * div_term)  # 奇数列为cos
    ...
    def forward(self, x):
        ...
        # 步骤1：提取对应序列长度的位置编码
        # self.pe的形状是(1, max_len, D)，自动广播到(N, S, D)
        positional_embeddings = self.pe[:, :S]  # 截取到当前序列长度
        
        # 步骤2：将位置编码叠加到输入序列
        output = x + positional_embeddings  # 元素级相加
        
        # 步骤3：应用dropout（原始代码遗漏的关键步骤）

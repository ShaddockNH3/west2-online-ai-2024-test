...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        loss_i=-torch.log(torch.exp(sim_pos / tau) / denom_k)

        denom_kN=0.0
        for m in range(2*N):
          if m==(k+N):
            continue
          denom_kN+=torch.exp(sim(z_k_N, out[m]) / tau)
        loss_j=-torch.log(torch.exp(sim_pos / tau) / denom_kN)
...
def sim_positive_pairs(out_left, out_right):
    ...
    dot_products = (out_left * out_right).sum(dim=1)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.mm(out, out.T) / (norms @ norms.T)
